...
export default function ExtractedProfile({ profile, totalMatched }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 mb-8 text-left">
      <div className="bg-slate-900/80 backdrop-blur-2xl border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-2xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-gradient-to-r from-orange-500/20 to-emerald-500/20 border border-orange-500/30 text-white shrink-0 self-start md:self-auto shadow-md">
            <Sparkles className="w-4 h-4 text-amber-300 animate-spin" />
            <span className="text-xs sm:text-sm font-bold">
              <span className="text-orange-400 font-black text-base">{totalMatched}</span> सरकारी योजनाएं पात्र हैं
            </span>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-800/80 flex flex-wrap items-center gap-2">
          ...
          {profile.extractedTags.map((tag, idx) => (
            <span
              key={idx}
              className="inline-flex items-center gap-1 px-3 py-1 rounded-xl text-xs font-bold bg-slate-800/90 text-slate-200 border border-slate-700 shadow-sm"
            >
              {tag}
            </span>
          ))}
