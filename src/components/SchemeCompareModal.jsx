    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-2xl my-8 text-left max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className="px-3 py-1 rounded-full bg-blue-100 border border-blue-300 text-blue-900 text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="p-3.5 bg-orange-50/60 rounded-2xl border border-orange-200">
            <label className="block text-xs font-bold text-orange-900 mb-1.5 uppercase">पहली योजना चुनें (Scheme 1):</label>
            <select
              value={scheme1Id}
              onChange={(e) => setScheme1Id(e.target.value)}
              className="w-full bg-white border border-orange-200 rounded-xl p-2.5 text-xs text-slate-900 focus:border-orange-500 font-bold shadow-sm"
            >
          ...
          <div className="p-3.5 bg-blue-50/60 rounded-2xl border border-blue-200">
            <label className="block text-xs font-bold text-blue-900 mb-1.5 uppercase">दूसरी योजना चुनें (Scheme 2):</label>
            <select
              value={scheme2Id}
              onChange={(e) => setScheme2Id(e.target.value)}
              className="w-full bg-white border border-blue-200 rounded-xl p-2.5 text-xs text-slate-900 focus:border-blue-500 font-bold shadow-sm"
            >
              {SCHEMES_DATABASE.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.hindiName} ({s.categoryLabel})
        ...
        <div className="border border-slate-200 rounded-2xl overflow-hidden text-xs shadow-sm">
          ...
          {/* Row 1: Header Titles */}
          <div className="grid grid-cols-2 bg-slate-100 border-b border-slate-200 p-4 font-black text-sm">
          ...
          <div className="grid grid-cols-2 p-4 border-b border-slate-200 bg-white gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">सरकारी लाभ (Total Benefit):</span>
              <span className="text-base font-black text-orange-700 block mt-0.5">{s1.benefit}</span>
              <p className="text-slate-600 text-[11px] mt-1">{s1.benefitDetail}</p>
            </div>
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">सरकारी लाभ (Total Benefit):</span>
              <span className="text-base font-black text-blue-700 block mt-0.5">{s2.benefit}</span>
              <p className="text-slate-600 text-[11px] mt-1">{s2.benefitDetail}</p>
            </div>
          </div>

          {/* Row 3: Eligibility & Age */}
          <div className="grid grid-cols-2 p-4 border-b border-slate-200 bg-slate-50/50 gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">पात्रता एवं उम्र सीमा:</span>
              <span className="text-slate-900 font-bold block mt-0.5">{s1.whoQualifies}</span>
              <span className="text-[11px] text-slate-500 block mt-1">उम्र: {s1.eligibility?.min_age || 18} से {s1.eligibility?.max_age || 100} वर्ष</span>
            </div>
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">पात्रता एवं उम्र सीमा:</span>
              <span className="text-slate-900 font-bold block mt-0.5">{s2.whoQualifies}</span>
              <span className="text-[11px] text-slate-500 block mt-1">उम्र: {s2.eligibility?.min_age || 18} से {s2.eligibility?.max_age || 100} वर्ष</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 p-4 border-b border-slate-200 bg-white gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">जमीन / संपत्ति अनिवार्यता:</span>
              <span className="font-bold block mt-0.5 text-slate-800">
          ...
          <div className="grid grid-cols-2 p-4 border-b border-slate-200 bg-slate-50/50 gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">जरूरी दस्तावेज:</span>
          ...
          <div className="grid grid-cols-2 p-4 bg-white gap-4 items-center">
            <div>
              <a
                href={s1.officialLink}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-xs font-bold text-orange-700 hover:text-orange-800 underline"
              >
