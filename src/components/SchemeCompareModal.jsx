    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-700/80 rounded-3xl p-6 sm:p-8 shadow-2xl my-8 text-left max-h-[92vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="p-3 bg-slate-950 rounded-2xl border border-slate-800">
            ...
            <select
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-white focus:border-orange-500 font-medium"
            >
          ...
          <div className="p-3 bg-slate-950 rounded-2xl border border-slate-800">
            <label className="block text-xs font-bold text-cyan-400 mb-1.5 uppercase">दूसरी योजना चुनें (Scheme 2):</label>
            <select
              value={scheme2Id}
              onChange={(e) => setScheme2Id(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-white focus:border-cyan-500 font-medium"
            >
              {SCHEMES_DATABASE.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.hindiName} ({s.categoryLabel})
        ...
        <div className="border border-slate-800 rounded-2xl overflow-hidden text-xs">
          ...
          {/* Row 1: Header Titles */}
          <div className="grid grid-cols-2 bg-slate-950 border-b border-slate-800 p-4 font-bold text-sm">
          ...
          <div className="grid grid-cols-2 p-4 border-b border-slate-800/80 bg-slate-900/50 gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">सरकारी लाभ (Total Benefit):</span>
              <span className="text-base font-black text-white block mt-0.5">{s1.benefit}</span>
              <p className="text-slate-300 text-[11px] mt-1">{s1.benefitDetail}</p>
            </div>
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">सरकारी लाभ (Total Benefit):</span>
              <span className="text-base font-black text-white block mt-0.5">{s2.benefit}</span>
              <p className="text-slate-300 text-[11px] mt-1">{s2.benefitDetail}</p>
            </div>
          </div>

          {/* Row 3: Eligibility & Age */}
          <div className="grid grid-cols-2 p-4 border-b border-slate-800/80 bg-slate-950/40 gap-4">
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">पात्रता एवं उम्र सीमा:</span>
              <span className="text-slate-200 font-semibold block mt-0.5">{s1.whoQualifies}</span>
            ...
            <div>
              <span className="text-slate-500 uppercase tracking-wider block text-[10px] font-bold">पात्रता एवं उम्र सीमा:</span>
              <span className="text-slate-200 font-semibold block mt-0.5">{s2.whoQualifies}</span>
          ...
          <div className="grid grid-cols-2 p-4 bg-slate-950 gap-4 items-center">
            <div>
              <a
